...
export default function App() {
  ...
  // Removed unused showPaymentModal warning by destructuring only the setter
  const [, setShowPaymentModal] = useState(false);
  const [activeLegalDoc, setActiveLegalDoc] = useState<string | null>(null); // 'privacy' | 'terms'
  ...
  const handleSend = async () => {
    ...
    setTimeout(() => {
      const newResponses = (isMultiChat ? activeModels : [activeModels[0]]).map(modelId => {
      ...
      if (isSuperFiesta) {
        setTimeout(() => {
          setMessages(prev => [...prev, {
            role: 'synthesis',
            content: `PRISM SYNTHESIS: Analyzing responses... Consensus suggests leveraging strengths of each perspective.`,
            timestamp: new Date()
          }]);
          setProcessing(false);
        }, 2000);
  ...
  const AttachMenu = ({ dropUp = true }) => (
    ...
    >
      <button className={`flex items-center gap-3 px-4 py-2.5 ${theme.hoverBg} ${theme.text} text-sm transition-colors`}><Paperclip className="w-4 h-4 text-emerald-500" /> Attach Files</button>
      <div className={`px-4 py-1.5 text-[10px] uppercase font-bold tracking-wider ${theme.textMuted}`}>Generate</div>
      <button className={`flex items-center gap-3 px-4 py-2.5 ${theme.hoverBg} ${theme.text} text-sm transition-colors`}><ImageIcon className="w-4 h-4 text-blue-400" /> Image</button>
      ...
      <button className={`flex items-center gap-3 px-4 py-2.5 ${theme.hoverBg} ${theme.text} text-sm transition-colors`}><Globe className="w-4 h-4 text-indigo-400" /> Web Search</button>
    </motion.div>
  );
    ...
    return (
      <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans selection:bg-emerald-500/30 scroll-smooth transition-colors duration-300`}>
               ...
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className={`absolute inset-0 rounded-xl border border-emerald-500/30 ${isDark ? 'bg-[#0f0f0f]' : 'bg-white'} shadow-2xl shadow-emerald-900/20 overflow-hidden flex flex-col items-center justify-center p-8`}>
                   ...
                   <div className="grid grid-cols-3 gap-4 w-full max-w-3xl">
                     {MODELS.map((m) => (
                       <div 
                         key={m.id} 
                         className={`${isDark ? 'bg-[#1a1a1a]' : 'bg-gray-50'} ${m.id === 'blackbox' ? 'col-start-2' : ''} p-3 rounded-lg border ${theme.border} flex items-center gap-3`}
                       >
           ...
           <div className="flex flex-col md:flex-row gap-6 items-center max-w-6xl mx-auto">
                <div className={`md:w-1/3 w-full ${isDark ? 'bg-[#0f0f0f]' : 'bg-white'} rounded-3xl p-8 border ${theme.border} text-left opacity-80 scale-95`}>
                     ...
                     <div className="space-y-4">
                         <div className="flex justify-between text-sm"><span className="flex items-center gap-2"><Bot className="w-4 h-4"/> ChatGPT 5</span> <span className="text-red-400">$20/mo</span></div>
                         <div className="flex justify-between text-sm"><span className="flex items-center gap-2"><Sparkles className="w-4 h-4"/> Google Gemini 2.5 Pro</span> <span className="text-red-400">$20/mo</span></div>
                         <div className="flex justify-between text-sm"><span className="flex items-center gap-2"><Globe className="w-4 h-4"/> Perplexity Sonar Pro</span> <span className="text-red-400">$20/mo</span></div>
                         <div className="flex justify-between text-sm"><span className="flex items-center gap-2"><Feather className="w-4 h-4"/> Claude Sonnet 4</span> <span className="text-red-400">$20/mo</span></div>
                         <div className="flex justify-between text-sm"><span className="flex items-center gap-2"><Cpu className="w-4 h-4"/> Blackbox AI</span> <span className="text-red-400">$20/mo</span></div>
                     </div>
                </div>
                    ...
                    <div className="bg-[#0a0a0a] rounded-[22px] p-8 h-full relative">
                        ...
                        <div className="flex gap-4 mb-8">
                            ...
                            <div className="flex-1 border border-emerald-500/50 bg-emerald-500/5 rounded-xl p-4 relative cursor-pointer">
                                 ...
                                 <div className="text-xs text-emerald-400 mt-1">Yearly (Save 25%) + Quarterly Webinar Access</div>
                        ...
                        <div className="mb-8">
                            ...
                            <p className="text-[10px] text-gray-500 mt-3 text-center leading-relaxed px-2">
                                By signing up you agree to receive verification emails, account updates and marketing communication related to AIFiesta. You can unsubscribe from marketing emails anytime. Read our <button onClick={() => setActiveLegalDoc('privacy')} className="underline hover:text-gray-300">Privacy Policy</button> and <button onClick={() => setActiveLegalDoc('terms')} className="underline hover:text-gray-300">Terms</button>.
                            </p>
                        </div>
        ...
        <AnimatePresence>
          {activeLegalDoc && (
            <LegalModal 
              title={activeLegalDoc === 'privacy' ? "Privacy Policy" : "Terms & Conditions"}
              content={activeLegalDoc === 'privacy' ? PRIVACY_CONTENT : TERMS_CONTENT}
  ...
  if (view === 'signin') {
      ...
      <div className={`min-h-screen ${theme.bg} flex items-center justify-center p-4 font-sans ${theme.text} relative overflow-hidden transition-colors duration-300`}>
        ...
        >
          <button onClick={() => setView('landing')} className={`absolute top-4 left-4 ${theme.textMuted} hover:${theme.text}`}>
          ...
          <button 
            ...
            className={`w-full ${isDark ? 'bg-[#1a1a1a] hover:bg-[#252525]' : 'bg-gray-100 hover:bg-gray-200'} ${theme.text} font-medium py-3 px-4 rounded-lg border ${theme.border} flex items-center justify-center gap-3 transition-all duration-200 mb-6 group`}
          >
          ...
          <div className="relative flex py-2 items-center mb-6">
            ...
            <span className={`flex-shrink-0 mx-4 text-xs ${theme.textMuted} uppercase tracking-widest`}>Or continue with email</span>
            <div className={`flex-grow border-t ${isDark ? 'border-gray-800' : 'border-gray-200'}`}></div>
          </div>
            ...
            <div className={`p-4 border-t ${theme.border} ${theme.sidebarBg}`}>
              <div className={`${theme.cardBg} rounded-xl p-4 border ${theme.border} mb-4`}>
                <div className="flex justify-between items-start mb-2">
                  <h3 className={`font-medium text-sm ${theme.text}`}>Free Plan</h3>
                </div>
                <div className={`text-xs ${theme.textMuted} mb-3`}>0 / 3 messages used</div>
          ...
          <div className="flex items-center gap-2">
             ...
             <div className="flex items-center gap-4 overflow-x-auto no-scrollbar max-w-[70vw] md:max-w-[60vw] lg:max-w-[70vw]">
               ...
               <button 
                ...
                className={`flex-shrink-0 w-8 h-8 rounded-full border border-dashed ${isDark ? 'border-gray-600 hover:bg-white/5' : 'border-gray-300 hover:bg-black/5'} flex items-center justify-center transition-all`}
               >
                ...
                <div className={`relative ${isDark ? 'bg-[#0f0f0f]' : 'bg-white'} rounded-xl flex items-center gap-3 p-2`}>
                    <div className="relative">
                        <button 
                            ...
                            className={`p-2 ${theme.hoverBg} rounded-lg ${theme.textMuted} transition-colors ${showAttachMenu ? theme.text : ''}`}
                        >
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button className={`p-2 ${theme.hoverBg} rounded-lg ${theme.textMuted}`}><ImageIcon className="w-4 h-4" /></button>
                        <button 
                            onClick={handleSend}
                            className={`p-2 rounded-lg transition-all ${input.trim() ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : `${theme.hoverBg} ${theme.textMuted} cursor-not-allowed`}`}
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {CARDS.map((card, i) => (
                        <motion.div 
                            ...
                            className={`${isDark ? 'bg-[#161616]' : 'bg-white'} border ${theme.border} rounded-2xl p-5 flex gap-5 cursor-pointer ${theme.hoverBg} transition-all group relative overflow-hidden shadow-sm`}
                        >
                    ...
                    if (msg.role === 'user') {
                        ...
                        return (
                             <motion.div 
                                ...
                                animate={{ opacity: 1, scale: 1 }}
                                className="col-span-full mt-8 border-t border-emerald-500/20 pt-8"
                             >
                                <div className={`bg-gradient-to-b ${isDark ? 'from-emerald-900/10 to-[#111]' : 'from-emerald-50 to-white'} border border-emerald-500/30 rounded-2xl p-6 relative overflow-hidden`}>
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <span className="font-bold text-emerald-500 tracking-wide text-sm">PRISM SYNTHESIS</span>
                                    </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex gap-2 mt-auto pt-3">
                                     <button className={`text-xs ${theme.textMuted} hover:${theme.text} flex items-center gap-1 ${theme.hoverBg} px-2 py-1 rounded`}><ImageIcon className="w-3 h-3" /> Copy</button>
                                     <button className={`text-xs ${theme.textMuted} hover:${theme.text} flex items-center gap-1 ${theme.hoverBg} px-2 py-1 rounded`}><Sparkles className="w-3 h-3" /> Improve</button>
                                </div>
                            </motion.div>
                        );
                    }
               ...
               <div className="absolute bottom-6 left-4 right-4 max-w-4xl mx-auto">
                    <div className={`relative ${isDark ? 'bg-[#1a1a1a]' : 'bg-white'} border ${theme.border} rounded-2xl p-2 shadow-2xl`}>
                        <div className={`relative ${isDark ? 'bg-[#0f0f0f]' : 'bg-white'} rounded-xl flex items-center gap-3 p-2`}>
                            ...
                            <button 
                                onClick={handleSend}
                                className={`p-2 rounded-lg transition-all ${input.trim() ? 'bg-emerald-500 text-white' : `${theme.hoverBg} ${theme.textMuted}`}`}
                            >
                                <Send className="w-4 h-4" />
                            </button>
                        </div>
          ...
          <>
            ...
            >
                ...
                <div className="p-4 space-y-2 max-h-[60vh] overflow-y-auto">
                    {MODELS.map((model) => (
                        <div key={model.id} className={`flex items-center justify-between p-3 ${theme.hoverBg} rounded-xl transition-colors group`}>
                            <div className="flex items-center gap-4">
                ...
                <div className={`p-4 ${isDark ? 'bg-[#111]' : 'bg-gray-50'} border-t ${theme.border} space-y-3`}>
                    ...
                    {!isPremium && (
                        <button onClick={() => setIsPremium(true)} className="w-full bg-black dark:bg-white text-white dark:text-black font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
                            <Zap className="w-4 h-4 fill-current" /> Upgrade and Unlock Premium AI Models
                        </button>
                    )}
                </div>
            </motion.div>
